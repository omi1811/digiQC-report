"""This code shows the data with eqc count of stages like how many checklists are pending
in pre stage, how many in during stage and how many in post stage.
Also an alert is generated at some set frequency.
If a checklist is in same stage for more than 3 weeks alert is generated.
If a checklist is in Redo for more than 3 days, alert is generated.
"""
...
# --- Load data ---
# The source `abc.csv` may be tab-separated; specify sep='\t' to avoid tokenization errors
...
# write only alerts file (user requested only alerts + wide building-wise report)
...
# Save
# do not write flattened reference file; skip writing EQC_Summary_WithTotals.csv
print("✅ Skipped writing flattened reference file; proceeding to prepare building-wise report")
...
try:
    ...
    if os.path.exists(src_path):
        # If a previous backup exists, overwrite it (only if bak_path provided)
        if bak_path:
            os.replace(src_path, bak_path)
        else:
            # no backup requested; leave existing file — it will be overwritten by to_csv
            pass
